...
--================================--
local menu = {
    enabled      = ui.new_checkbox("LUA", lua_container, "Minesweeper"),
    difficulty   = ui.new_combobox("LUA", lua_container, "Difficulty", "Easy", "Medium", "Hard", "Custom"),
    rows         = ui.new_slider("LUA", lua_container, "Rows", 1, 100, 16),
    columns      = ui.new_slider("LUA", lua_container, "Columns", 1, 100, 30),
    mines        = ui.new_slider("LUA", lua_container, "Mines", 1, 1000, 99),
    edit         = {
        enabled = ui.new_checkbox("LUA", lua_container, "Settings"),
        reveal       = ui.new_hotkey("LUA", lua_container, "Reveal tile", false, 0x01),
        flag         = ui.new_hotkey("LUA", lua_container, "Flag tile", false, 0x02),
        tile_w  = ui.new_slider("LUA", lua_container, "Tile width", 5, 100, 24, true, "px"),
        x       = ui.new_slider("LUA", lua_container, "X", 0, screen_width, 300, true, "px"),
        y       = ui.new_slider("LUA", lua_container, "Y", 0, screen_height, 200, true, "px")
    },
    reset
}
...
local function generate_map()
    handle_menu() --  bro..... i putting this shits here because I cant doing ui.set_callback(menu.difficulty, handle_menu) and ui.set_callback(menu.difficulty, generate_map). IMPOSSIBLE!!!!
...
-- Reveal tile
local function reveal_tile(selected_row, selected_col)
...
menu.reset = ui.new_button("LUA", lua_container, "Reset", generate_map)
    ...
    if ui.get(menu.enabled) and ui.is_menu_open() then
        ...
        if selected_tile ~= nil and game_state == 0 then
            ...
            if ui.get(menu.edit.reveal) then
                reveal_tile(selected_row, selected_col)
